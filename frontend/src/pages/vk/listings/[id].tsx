...
export default function VKListingDetail() {
  const router = useRouter();
  const { id } = router.query;
  const { hapticFeedback, share } = useVK();
  ...
  const handleContact = async () => {
    ...
    try {
      ...
      router.push(`/vk/messages/${response.data._id}`);
    } catch (error) {
  ...
  const handleShare = () => {
    hapticFeedback('success');
    share(`${listing?.title} - ${formatPrice(listing?.price || 0)}`);
  };
